...
func TestValidateBlockHeader(t *testing.T) {
	...
	reveal := makeReveal(state, state.Validators.Proposer.Address, privVals, nextHeight)
	block, err := state.MakeBlock(nextHeight, test.MakeNTxs(state.LastBlockHeight, 10), lastCommit, nil, reveal, state.Validators.Proposer.Address)
	require.NoError(t, err)
	...
	for height := int64(1); height < validationTestsStopHeight; height++ {
		...
		if height > 1 {
			...
			*/
			...
			block, err := state.MakeBlock(height, test.MakeNTxs(height, 10), lastCommit, evidence, reveal, proposerAddr)
			require.NoError(t, err)
			err = blockExec.ValidateBlock(state, block, false)
...
func TestValidateBlockTime(t *testing.T) {
	...
	t.Run("block time before last block time", func(t *testing.T) {
		height := int64(3)
		block, err := makeBlockWithReveal(state, height, lastCommit, privVals)
		require.NoError(t, err)
	...
	t.Run("block time exceeds wall clock tolerance", func(t *testing.T) {
		...
		err = blockExecWithTol.ValidateBlock(state, block, false)
		require.Error(t, err)
	...
	t.Run("tolerance not set still allows valid blocks", func(t *testing.T) {
		...
		block, err := makeBlockWithReveal(state, height, lastCommit, privVals)
		require.NoError(t, err)
		err = blockExecNoTol.ValidateBlock(state, block, false)
		require.NoError(t, err)
	})
}
